...
const PropertyDetails = ({ propertyId, propertyMarket, searchProperty, myProperty, propertyType, history }) => {
  ...
  useEffect(() => {
    if (myProperty) {
      ...
      axios.get(apiUrl + 'ws/listings/search?market=gsmls&extended=true&details=true&zip=' + myProperty.xf_postalcode, config).then((res) => {
         ...
         */}
                  ...
                  relatedResult.map((singleRelated, index) => {
                    return (
                      <div className='mx-1' key={index}>
                        <PropertyCard propertyValues={singleRelated} history={history} onCardClick={onCardClick} />
                      </div>
                    );
                  })}
              </Carousel>
          ...
          <div className='home-details-listing-provided-by'>
            <div className='bUREIR'>
              ...
              <p className='bDTsLB'>
                IDX information is provided exclusively for personal, non-commercial use, and may not be used for any purpose other than to identify prospective properties consumers may be interested
                in purchasing. Information is deemed reliable but not guaranteed.
              </p>
            </div>
          </div>
